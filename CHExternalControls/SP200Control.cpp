...
CSP200Control::CSP200Control(int nIndex, int nControlType) : CLightControl(nIndex, nControlType)
{
	m_nPortIndex	= 1;
	m_nBaudRate		= 9600;
	m_bConnected	= FALSE;
...
CSP200Control::~CSP200Control(void)
{
	Disconnect();
}

void CSP200Control::SetMappingValue(int nMin, int nMax, int nMinMap, int nMaxMap)
{
	m_dLightResolution	= 1.0;
...
BOOL CSP200Control::Connect(const CString& strPort, int nBaud)
{
	if(m_nPortIndex < 1) return FALSE;

	int nPort = _ttoi(strPort);

	m_bConnected = InitPort(NULL, nPort, nBaud, 'N', 8, 1);
...
BOOL CSP200Control::GetValue(int &nValue)
{
	if (!m_bConnected) return FALSE;
...
BOOL CSP200Control::GetValue(double &dValue)
{
	if (!m_bConnected) return FALSE;
...
BOOL CSP200Control::SetValue(int nValue)
{
	if (!m_bConnected) return FALSE;
...
BOOL CSP200Control::SetValue(double dValue)
{
	if (!m_bConnected) return FALSE;
...
//
BOOL CSP200Control::InitPort(CWnd* pPortOwner,	// the owner (CWnd) of the port (receives message)
...
//
UINT CSP200Control::CommThread(LPVOID pParam)
{
	// Cast the void pointer passed to the thread back to
	// a pointer of CSerialPort class
	CSP200Control *port = (CSP200Control*)pParam;
...
// start comm watching
//
BOOL CSP200Control::StartMonitoring()
{
	m_bThreadRun = TRUE;
...
//
BOOL CSP200Control::RestartMonitoring()
{
...
//
BOOL CSP200Control::StopMonitoring()
{
	m_bThreadRun = FALSE;
...
//
void CSP200Control::ProcessErrorMessage(CString ErrorText)
{
...
// Write a character.
//
void CSP200Control::WriteChar(CSP200Control* port)
{
	BOOL bWrite = TRUE;
	BOOL bResult = TRUE;
...
//
void CSP200Control::ReceiveChar(CSP200Control* port, COMSTAT comstat)
{
	BOOL  bRead = TRUE; 
	BOOL  bResult = TRUE;
	DWORD dwError = 0;
...
//
void CSP200Control::WriteToPort(unsigned char* Data, UINT Size)
{		
	ASSERT(m_hComm != 0);
...
void CSP200Control::ClearReadBuff(void)
{
...
//
DWORD CSP200Control::GetCommEvents()
{
...
//
DWORD CSP200Control::GetWriteBufferSize()
{
