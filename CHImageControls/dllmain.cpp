...
static AFX_EXTENSION_MODULE CHImageControlsDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("CHImageControls.DLL Init!\n");
		
		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(CHImageControlsDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(CHImageControlsDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("CHImageControls.DLL Deinit!\n");

		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(CHImageControlsDLL);
	}
	return 1;   // 확인
}
